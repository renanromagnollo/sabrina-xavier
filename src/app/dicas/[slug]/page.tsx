...
export default function DicaPage({params}: DicaPageProps){
    ...
    const [relatedProducts, setRelatedProducts] = useState<HygraphProductProps[]>()
    ...
    useEffect(() => {
        ...
        const getPosts = async () => {
            ...
            const selected: HygraphPostProps = posts.filter(post => verifySlug(post))[0]
        ...
        <PageArea>
            ...
            <RightArea>
                { relatedProducts && relatedProducts.length > 0 && 
                    (
                        <ProductsArea>
                            <AreaTitle>{relatedProducts.length > 1 ? 'Produtos Recomendados' : 'Produto Recomendado'}</AreaTitle>
                            {relatedProducts?.map((product, i) => <ProductCard key={i} item={product}/>)}
                        </ProductsArea>
                    )
                }
            </RightArea>
